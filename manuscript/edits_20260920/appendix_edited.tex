...
%% [CLAUDE 2026-09-20]  - Jaccard beats overlap on joint NMI only under split (+0.05) and merge (+0.06), paired 95% CIs exclude zero; ties elsewhere (within 0.01). Overlap never wins on membership accuracy; it wins on K MAE in Table 2 because it links more aggressively.
...
%% [CLAUDE 2026-09-20]  - Design: 594 configurations x 50 simulations x B = 100 (sim/02 stability), Jaccard + Leiden; odd/even split by configuration.
...
%% [CLAUDE 2026-09-20]  - Robustness arms for the SAME quantity: degree-corrected generator (sim/03, pending) and weighted networks; report whether the validation share stays near 0.95.
%% [CLAUDE 2026-09-20]  - Leave-one-level-out calibration (drop each n, each density level in turn) as the extrapolation check (post, pending).
%% [CLAUDE 2026-09-20]  - Selection-rule check: on the regime grid, does the method with the higher stability score have the higher accuracy? (sim/06 selection, pending).
%% [CLAUDE 2026-09-20]  - Empirical stability scores and floors for the four networks (empirical/10, pending).
...
%% [CLAUDE 2026-09-20] omega sweep (0.25, 0.5, 1, 2, 4) on the regime grid with the corrected multislice is sim/05 (pending); figure name fig_omega_sweep.pdf. The gamma part of the caption can stay if you also sweep gamma; otherwise rename the section to 'Multislice Coupling Sensitivity'.
