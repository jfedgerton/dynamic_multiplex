...
%% [CLAUDE 2026-09-22] fig_order_perturb.png and the alternative-alignment table are NOT produced by replication/; keep this section only if you can regenerate them from a script in the repo, otherwise delete it (the conclusion limitation 3 pointer too). Figure commented out so the file compiles.
% \begin{figure}[htbp]\centering
...
\label{app:runtime}
%% [CLAUDE 2026-09-22] no runtime table is produced by replication/; either compute one from the runtime_s columns in output/regime (I can add a post script) or drop the section and the main-text pointer.
\begin{table}[ht]\centering
