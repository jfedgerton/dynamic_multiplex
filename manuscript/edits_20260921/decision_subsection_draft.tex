%% ============================================================================
%% DRAFT for Jared, 2026-09-21. A main-text subsection: when to use which
%% method. Numbers are from the Sept 21 front-to-end rerun (v1.3.0,
...
accurate method in 91\% of series), and when structure recurs on a period
...
%% [CLAUDE 2026-09-21] Abrupt rewiring: in the 30-replicate Table 2 run DynMux and multislice tie (0.489 vs 0.487, paired p = 0.59); in the 10-replicate omega sweep on the same seeds DynMux leads by 0.10. The gap is optimiser noise in the multislice fit at n <= 100 (see the note I sent you). Until that is settled, do not list abrupt rewiring as a DynMux win; the safe sentence is "at an abrupt change-point the two are indistinguishable on joint NMI, and DynMux has the lower K error (1.35 vs 1.26 is actually multislice lower; check tab_metrics_wide)".
...
Multislice wins by wide margins under gradual switching (0.54 against 0.27),
under a stable core with a rotating periphery (0.87 against 0.46), and on
...
every case (Appendix~X): its stability scores are 0.97, 0.75, 0.80 and 0.59
