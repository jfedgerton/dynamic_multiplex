# =============================================================================
# replication/sim/04_coverage_misspec.R   (formerly manuscript/19_coverage_misspec.R)
#
...
# computable exactly as in 02_coverage_grid.R.
#
...
#
# Usage (mini test): COV_TASK=1 COV_MINI=1 DM_ROOT=. Rscript replication/sim/04_coverage_misspec.R
# =============================================================================
...
outdir <- file.path(ROOT, "output", "coverage_misspec")
...
calib <- data.frame(task = TASK, n = cfg$n, K = cfg$K, p_switch = cfg$p_switch, density = cfg$density, T_ = cfg$T_, hetero = cfg$hetero, balance = cfg$balance, fit_type = cfg$fit_type, algorithm = cfg$algorithm,
                    bin = seq_len(N_BINS),
...
joint_df <- data.frame(task = TASK, n = cfg$n, K = cfg$K, p_switch = cfg$p_switch, density = cfg$density, T_ = cfg$T_, hetero = cfg$hetero, balance = cfg$balance, fit_type = cfg$fit_type, algorithm = cfg$algorithm,
