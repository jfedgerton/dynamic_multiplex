...
class TestArbitraryNodeLabels:
    ...
    def test_node_labels_mapping_returned(self):
        """Relabeled fits should expose original labels: index i -> node id i+1."""
        labels = ["alice", "bob", "carol", "dave"]
        ...
        out = fit_multilayer_jaccard([g1, g2], algorithm="louvain")
        assert out["node_labels"] == sorted(labels)

    def test_node_labels_none_for_canonical_input(self):
        """Adjacency-matrix input needs no relabeling, so node_labels is None."""
        out = fit_multilayer_jaccard([np.eye(5), np.eye(5)], algorithm="louvain")
        assert out["node_labels"] is None
