# =============================================================================
# replication/sim/02_coverage_grid.R   (formerly manuscript/16_coverage3_grid.R)
#
...
# the v2 sweep runs.
#
# Outputs per task (all under $DM_ROOT/output/coverage_grid/):
#   cov_task*.csv    one row per simulation: coverage + width summaries
#   calib_task*.csv  50 phat bins: n_pairs, n_true (planted same-community),
#                    n_wilson_covered (Wilson interval covered p_star)
#   joint_task*.csv  50x50 (phat bin, pstar bin) counts -- the input for
#                    the calibrated interval and for conditional-coverage checks
...
# propensity vs R_TRUTH fresh-draw ground truth), 50-bin calibration.
#
# Usage (mini test): COV_TASK=1 COV_MINI=1 DM_ROOT=. Rscript replication/sim/02_coverage_grid.R
# =============================================================================
...
ALPHA   <- 0.05
N_BINS  <- 50L   # 0.02-wide bins: sets the resolution of the calibrated interval (post/14)
...
outdir <- file.path(ROOT, "output", "coverage_grid")
dir.create(outdir, showWarnings = FALSE, recursive = TRUE)

# ------------------- fixed grid, deterministically shuffled ------------------
cells <- expand.grid(
...
if (nzchar(Sys.getenv("SLURM_ARRAY_TASK_ID"))) {
  done <- file.exists(file.path(outdir, sprintf("cov_task%05d.csv", TASK))) &&
          file.exists(file.path(outdir, sprintf("calib_task%05d.csv", TASK))) &&
          file.exists(file.path(outdir, sprintf("joint_task%05d.csv", TASK)))
  if (done) { cat("[skip] task", TASK, "already complete\n"); quit(save = "no") }
}
...
cat(sprintf("[coverage3grid] task=%d/%d cell=%d : n=%d K=%d sw=%.2f dens=%s T=%d %s/%s M=%d B=%d R=%d\n",
            TASK, nrow(task_tab), perm[TASK], cfg$n, cfg$K, cfg$p_switch,
            cfg$density, cfg$T_, cfg$fit_type, cfg$algorithm,
...
run_one <- function(sim_id) {
  seed <- (15000L + TASK) * 100000L + sim_id
  set.seed(seed)
  ...
  bin_wcov  <- integer(N_BINS)                 # Wilson interval covered p_star, by phat bin
  joint     <- integer(N_BINS * N_BINS)        # (phat bin, pstar bin) counts, column-major
  ...
  for (t in seq_len(T_)) {
    ...
    binS <- pmin(N_BINS, 1L + floor(pstar * N_BINS))
    bin_total <- bin_total + tabulate(bin, nbins = N_BINS)
    bin_true  <- bin_true  + tabulate(bin[same], nbins = N_BINS)
    bin_wcov  <- bin_wcov  + tabulate(bin[covp], nbins = N_BINS)
    # joint distribution of (phat, pstar): the input for a calibrated interval
    # whose width does not depend on B. Index = phat bin + N_BINS * (pstar bin - 1).
    joint <- joint + tabulate(bin + N_BINS * (binS - 1L), nbins = N_BINS * N_BINS)
  }
...
bwcov <- Reduce(`+`, lapply(res, `[[`, "bin_wcov"))
jnt   <- Reduce(`+`, lapply(res, `[[`, "joint"))
stopifnot(length(btot) == N_BINS, length(jnt) == N_BINS * N_BINS)
stopifnot(sum(jnt) == sum(btot))          # every (phat, pstar) pair lands in exactly one cell

# calibration bins: self-describing (design columns restored so rows are
# attributable without replaying the seed-123 shuffle)
calib <- data.frame(task = TASK, n = cfg$n, K = cfg$K, p_switch = cfg$p_switch, density = cfg$density, T_ = cfg$T_, weights = cfg$weights, fit_type = cfg$fit_type, algorithm = cfg$algorithm,
                    bin = seq_len(N_BINS),
                    bin_lo = (seq_len(N_BINS) - 1) / N_BINS,
                    bin_hi = seq_len(N_BINS) / N_BINS,
                    n_pairs = btot, n_true = btrue, n_wilson_covered = bwcov,
                    stringsAsFactors = FALSE)

# joint (phat bin, pstar bin) counts: long form, 2500 rows per task
joint_df <- data.frame(task = TASK, n = cfg$n, K = cfg$K, p_switch = cfg$p_switch, density = cfg$density, T_ = cfg$T_, weights = cfg$weights, fit_type = cfg$fit_type, algorithm = cfg$algorithm,
                       bin_phat  = rep(seq_len(N_BINS), times = N_BINS),
                       bin_pstar = rep(seq_len(N_BINS), each  = N_BINS),
                       count = jnt, stringsAsFactors = FALSE)

write.csv(rows,     file.path(outdir, sprintf("cov_task%05d.csv",   TASK)), row.names = FALSE)
write.csv(calib,    file.path(outdir, sprintf("calib_task%05d.csv", TASK)), row.names = FALSE)
write.csv(joint_df, file.path(outdir, sprintf("joint_task%05d.csv", TASK)), row.names = FALSE)

cat(sprintf("[coverage3grid] task %d done: %d/%d sims ok, %.1f min. cov_K=%.3f cov_P=%.3f (same=%.3f diff=%.3f)\n",
