...
GLc<-lapply(S$graph_layers,function(g){d<-igraph::degree(g);igraph::delete_vertices(g,which(d==0))})
...
comp<-function(fn)function(){d<-fn(simU);if(is.null(d))return(NULL);lapply(seq_along(d),function(t){a<-U$active[[t]];setNames(as.integer(d[[t]][a]),U$names[a])})}
METHODS<-list(
