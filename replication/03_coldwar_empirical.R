#!/usr/bin/env Rscript
#
# replication/03_coldwar_empirical.R
#
# EMPIRICAL / COLD-WAR APPLICATION (alliance, IGO, trade).
#
# This is the exact code and seeds behind the empirical section of the paper.
# It applies dynamic_multiplex to temporal international-relations networks
# constructed from the peacesciencer R package:
#   - Alliance networks  (ATOP defense pacts)  - the showcase: crisp Cold-War
#                                                break in community persistence
#   - IGO networks       (shared IGO memberships) - second witness to the break
#   - Trade networks     (COW bilateral trade)    - specificity check: gradual
#                                                   drift, no sharp break
#
# Each network type is a sequence of yearly layers (YEAR_RANGE); the community
# structure is tracked over time and its year-to-year persistence is measured.
# The Cold-War window is read off the persistence trajectory per network.
#
# Extended analysis:
...
#   - Measures community persistence across time (the Cold-War diagnostic)
#
# Note: this is the untouched analysis script used in the study, with only the
# output directory redirected to the replication tree. Confidence intervals are
# not drawn on the empirical figures (per the study design); the CI machinery
# is validated separately in 01_ci_coverage.R.
#
# Outputs (replication/output/empirical/):
...
#   Rscript replication/03_coldwar_empirical.R
...
outdir <- Sys.getenv("EMP_OUTROOT", "replication/output/empirical")
