...
%% [CLAUDE 2026-09-22]  - Leave-one-level-out, REVISED after the Codex check: a held-out fit whose stability bin has no calibration fits at or below it has no floor. post/12 now reports the share above the floor among held-out fits that have one, plus the count with one (new column). The weak-density level is the case that matters: 5,792 of its 9,900 fits sit below the calibrated range when the weak arm is held out, and among the 4,108 with a floor the share is 0.203 (confirmed, regenerated tab_app_stability_lolo.tex 2026-09-22). Every other held-out level has a floor for all its fits (n = 50: 5,400; n = 100/200/400: 8,100; density default/strong: 9,900; K = 3/5: 10,800; K = 10: 8,100), so only the weak-density row changes. Statement for the text: the floor is a within-range guarantee; it does not extrapolate to stability levels never seen in calibration, and the weak-density hold-out is exactly that case. The other levels are unaffected (no empty-bin lookups).
...
%% [CLAUDE 2026-09-22] Codex check (task 2): the second sentence is wrong. sim/02 draws each bootstrap replicate from the fitted PLAIN block model (block densities only, no theta), so degree heterogeneity is in the data but NOT in the resampling model; that mismatch is exactly the misspecification this arm tests. Suggested replacement: "For the simulations, $\theta$ is fixed per simulated network. The parametric bootstrap resamples from the fitted plain block model, which carries no degree propensities, so the degree heterogeneity in the data is absent from the resampling model; this mismatch is the misspecification the arm tests."
...
\begin{table}[ht]
\centering\small
\caption{Validation of the binary-calibrated floor across simulation arms: number of fits, number whose stability bin has a calibrated floor, share of those above the floor, Spearman correlation of stability and accuracy, mean stability and mean accuracy.}
...
\begin{table}[ht]
\centering\small
\caption{Leave-one-level-out validation: the floor is recalibrated with each network size, density or community-count level excluded, then applied to the held-out level. Held-out fits whose stability bin has no calibration fits at or below it have no floor and are counted in the fourth column; the share above the floor is computed among the remaining fits.}
...
\begin{table}[ht]
\centering\small
\caption{Recovery of coded international orders by network and method. In each year the active orders are assigned to distinct communities by a Hungarian assignment maximising total Jaccard, so a community can recover at most one order; unassigned orders score zero. Cells are the Jaccard index, precision and recall of the assigned community and the number of communities $K$ among the active states, averaged over all order-years of the network.}
...
\begin{table}[ht]
\centering\scriptsize
\caption{Recovery of each coded order in the alliance (ATOP) network, by method: precision / recall of the community assigned to the order (one-to-one assignment within each year), averaged over the order's active years.}
...
\begin{table}[ht]
\centering\scriptsize
\caption{Recovery of each coded order in the defense cooperation (DCA) network, by method: precision / recall of the community assigned to the order (one-to-one assignment within each year), averaged over the order's active years.}
...
\begin{table}[ht]
\centering\scriptsize
\caption{Recovery of each coded order in the IGO network, by method: precision / recall of the community assigned to the order (one-to-one assignment within each year), averaged over the order's active years.}
...
\begin{table}[ht]
\centering\scriptsize
\caption{Recovery of each coded order in the trade network, by method: precision / recall of the community assigned to the order (one-to-one assignment within each year), averaged over the order's active years.}
